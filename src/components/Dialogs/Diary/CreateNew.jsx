                  ...
                  <form onSubmit={handleSubmit(onCreate)}>
                    <div className="flex flex-col w-full space-y-2">
                      <div className="flex items-center w-full px-3 rounded-lg bg-[#1F1F1F]">
                        ...
                        <input type="text" name="photo" placeholder="Profile URL" className="w-full h-full px-3 py-4 bg-[#1F1F1F] text-modern-white focus:outline-none disabled:cursor-not-allowed disabled:opacity-50" {...register("photo", { required: true, pattern: { value: /(http(s)?:\/\/.)?(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/g, message: "Invalid URL" } })} disabled={isSubmitting} />
                        {errors.photo && <span className="flex flex-row justify-end text-[10px] text-honey">{errors.photo.message || 'Required'}</span>}
                      </div>
                      <div className="flex items-center w-full px-3 rounded-lg bg-[#1F1F1F]">
                        <svg className="w-8 h-8 opacity-40" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
