...
export default function UpdateContact({ online_user, id, profile, name, phone, email, address, facebook, instagram, twitter, tiktok, youtube }) {
  ...
  return (
    <>
      <button
        className="text-gray-500 transition ease-in-out duration-200 rounded-full hover:scale-95 focus:outline-none"
        type="button"
        onClick={openModal}
      >
                    ...
                    <div className="flex flex-col md:flex-row w-full space-y-3 space-x-0 md:space-y-0 md:space-x-3">
                      <div className="flex flex-col w-full space-y-3">
                        <div className="flex items-center w-full px-3 rounded-lg bg-[#1F1F1F]">
                          ...
                          <input type="text" name="profile" placeholder="Profile URL" className="w-full h-full px-3 py-4 bg-[#1F1F1F] text-modern-white focus:outline-none disabled:cursor-not-allowed disabled:opacity-50" {...register("profile", { required: true, pattern: { value: /(http(s)?:\/\/.)?(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/g, message: "Invalid URL" } })} disabled={isSubmitting} />
                          {errors.profile && <span className="flex flex-row justify-end text-[10px] text-honey">{errors.profile.message || 'Required'}</span>}
                        </div>
                        <div className="flex items-center w-full px-3 rounded-lg bg-[#1F1F1F]">
                          <svg className="w-8 h-8 opacity-40" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div className="flex items-center w-full px-3 rounded-lg bg-[#1F1F1F]">
                          ...
                          <input type="text" name="email" placeholder="Email" className="w-full h-full px-3 py-4 bg-[#1F1F1F] text-modern-white focus:outline-none disabled:cursor-not-allowed disabled:opacity-50" {...register("email", { pattern: { value: /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i, message: "Invalid Email" } })} disabled={isSubmitting} />
                          {errors.email && <span className="flex flex-row justify-end text-[10px] text-honey">{ errors.email.message }</span>}
                        </div>
                        <div className="flex items-center w-full px-3 rounded-lg bg-[#1F1F1F]">
                          <svg className="w-8 h-8 opacity-40" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
